...
if not exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[Plan]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
...
(
	...
	Amount money,
	Description varchar(500),
	constraint fk_BillPlanCat FOREIGN KEY (BillCategoryID) REFERENCES [dbo].BillCategory(id),
	constraint fk_BillPlanPurp FOREIGN KEY (BillPurposeID) REFERENCES [dbo].BillPurpose(id)
)
